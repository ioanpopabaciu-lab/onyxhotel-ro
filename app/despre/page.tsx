    ...
    <>
      ...
      <section style={{ position: "relative", height: "45vh", minHeight: 340, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "0 2rem" }}>
          ...
          <h1 className="section-title" style={{ fontSize: "clamp(2.5rem, 6vw, 4.5rem)" }}>ONYX B&B Hotel & Restaurant</h1>
      ...
      <section style={{ padding: "7rem 2rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }}>
          <div>
            ...
            <h2 className="section-title" style={{ fontSize: "clamp(1.8rem, 3vw, 2.5rem)", marginBottom: "0.5rem" }}>
              La frontiera România–Ungaria
            </h2>
            <div className="gold-line-left" />
            <p style={{ fontFamily: "var(--font-inter)", color: "#aaa", fontSize: "0.9rem", lineHeight: 1.9, marginTop: "1.5rem" }}>
              ONYX B&B Hotel & Restaurant este amplasat la <strong style={{ color: "var(--cream)" }}>100 de metri de Punctul de Trecere a Frontierei Borș</strong> (Vama Borș), direct pe șoseaua europeană E60. Această poziție unică face din hotelul nostru prima alegere pentru:
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem", margin: "1.5rem 0 2rem" }}>
              {[
                ...
                <div key={item} style={{ display: "flex", gap: "0.8rem", alignItems: "flex-start" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-inter)", color: "#bbb", fontSize: "0.85rem" }}>{item}</span>
                </div>
              ))}
            </div>
          ...
          <div style={{ position: "relative", height: 480 }}>
            ...
            <div style={{ position: "absolute", top: "1.5rem", left: "1.5rem", background: "var(--gold)", padding: "1.2rem 1.5rem" }}>
              <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.65rem", fontWeight: 700, color: "var(--anthracite)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.2rem" }}>Distanță frontieră</p>
              <p style={{ fontFamily: "var(--font-playfair)", fontSize: "2rem", fontWeight: 900, color: "var(--anthracite)" }}>100m</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "2px" }}>
            {[
              ...
              { value: "2", label: "Săli conferință" },
              { value: "24/7", label: "Recepție deschisă" },
              { value: "E60", label: "Acces direct" },
            ].map((s) => (
              <div key={s.label} style={{ background: "#252525", padding: "2rem 1rem" }}>
                <p style={{ fontFamily: "var(--font-playfair)", fontSize: "2rem", fontWeight: 900, color: "var(--gold)", marginBottom: "0.4rem" }}>{s.value}</p>
                <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.72rem", color: "#888", textTransform: "uppercase", letterSpacing: "0.1em" }}>{s.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "7rem 2rem", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "2rem" }}>
          {[
            ...
            { icon: "🤝", title: "Conferințe", desc: "Două săli echipate în mansardă, pachete corporate all-inclusive, ideal pentru training și ședințe." },
            ...
            <div key={s.title} style={{ padding: "2rem", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ fontSize: "2.2rem", marginBottom: "1rem" }}>{s.icon}</div>
              <h3 style={{ fontFamily: "var(--font-playfair)", fontSize: "1.3rem", color: "var(--cream)", marginBottom: "0.7rem" }}>{s.title}</h3>
              <p style={{ fontFamily: "var(--font-inter)", color: "#888", fontSize: "0.82rem", lineHeight: 1.8 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
